...
Cdist= {'normal'; 'exponential'; 'gamma'; 'logistic'; ...
		  ...
		  'gp'; 'loglogistic'; 'lognormal'; 'weibull';'stable'};
mustbepos= 11;
	...
	for j= 1:numel(Cdist)
		...
		% Any other case do the fit ...
			...
			try
				...
				elseif numel(F(j).par) == 3
					pdfv= pdf(F(j).name,data,F(j).par(1),F(j).par(2),F(j).par(3));
				else
					pdfv= pdf(F(j).name,data,F(j).par(1),F(j).par(2),F(j).par(3),F(j).par(4));
				end
				F(j).LL=  sum(log(pdfv(pdfv>0 & ~isinf(pdfv))));
...
% Nice screen output
if strcmp('on',output)
	fprintf('\n\t\t\t\tName\t\tPar1\t\tPar2\t\tPar3\t\tPar4\t\tLogL\t\tAIC\n')
	for j= 1:size(F,2)
